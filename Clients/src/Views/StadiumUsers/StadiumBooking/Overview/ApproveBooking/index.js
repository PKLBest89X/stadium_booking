...
import { onLoadPrePendingBooking, onShowBookingDetails } from "../../../../../Slices/Features/StadiumUsers/BookingForNoAccount/preBookingNonAccountSlice";
import NonApproveBooking from './NonApproveBooking'
...
const ApproveBooking = React.memo(() => {
  ...
  useEffect(() => {
    const getReportBooking = async () => {
      ...
      dispatch(onLoadPrePendingBooking("pending"));
    };
    getReportBooking();
  ...
  return (
    <ReportContainerLayout>
      ...
      <Divider />
      <Box padding="1rem">
        {unActiveApproveSuccess === true &&
          unActiveApproveData.map((items, index) => {
            return (
        ...
        {unActiveApproveSuccess === false && <NonApproveBooking />}
      </Box>
    </ReportContainerLayout>
  );
});
