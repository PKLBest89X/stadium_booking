...
import NonActiveApproveBooking from "./NonActiveApproveBooking";
...
const ActiveApproveBooking = React.memo(() => {
  ...
  const { activeApproveData, activeApproveSuccess } =
  ...
  useEffect(() => {
    const getReportBooking = async () => {
      ...
      dispatch(onLoadPreActiveBooking("ຍັງບໍ່ເຕະ"));
    };
    getReportBooking();
  ...
  return (
    <ReportContainerLayout>
      ...
      <Divider />
      <Box padding="1rem">
        {activeApproveSuccess === true &&
          activeApproveData.map((items, index) => {
            return (
        ...
        {activeApproveSuccess === false && <NonActiveApproveBooking />}
      </Box>
    </ReportContainerLayout>
  );
});

export default ActiveApproveBooking;
