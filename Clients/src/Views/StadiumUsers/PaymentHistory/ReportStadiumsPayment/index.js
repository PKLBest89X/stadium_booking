...
const ReportStadiumsPayment = React.memo(() => {
  const classes = useStyles();
  const dispatch = useDispatch();
  const { checkResult } = useShallowEqualSelector((state) => state.validData);
  const { stadiumsOnlyData, stadiumsOnly } = useShallowEqualSelector(
  ...
  useEffect(() => {
    const fetchingPaymentDetails = async () => {
      ...
      dispatch(onLoadAdminOnlyStadiumsPayment());
    };
  ...
  return (
    <ReportContainerLayout>
      ...
      <Box padding="1rem">
        {stadiumsOnly === true &&
          stadiumsOnlyData.map((items, index) => {
            return (
        ...
        {stadiumsOnly === false && <NonPaymenthistory />}
      </Box>
    </ReportContainerLayout>
  );
});

export default ReportStadiumsPayment;
