...
const ReportAllPayment = React.memo(() => {
  const classes = useStyles();
  const dispatch = useDispatch();
  const { checkResult } = useShallowEqualSelector((state) => state.validData);
  const { reportStadiumsData, reportStadiumsSuccess } = useShallowEqualSelector(
  ...
  return (
    <ReportContainerLayout>
      ...
      <Divider />
      <Box padding="1rem">
        {reportStadiumsSuccess === true &&
          reportStadiumsData.map((items, index) => {
            return (
...
export default ReportAllPayment;
