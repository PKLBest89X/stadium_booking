...
const VoidBooking = React.memo(() => {
  ...
  const { userVoidData, userVoidSuccess, resultSearchAndSeletedDate } =
  ...
  useEffect(() => {
    const getReportBooking = async () => {
      ...
      if (userToken && userToken.token) {
        ...
        dispatch(onLoadUserVoidBooking("void"));
      }
    };
    getReportBooking();
  }, [dispatch]);
  ...
  return (
    <ReportContainerLayout>
      ...
      <Box padding="1rem">
        ...
        {userVoidSuccess === false && <NonVoidBooking />}
      </Box>
    </ReportContainerLayout>
  );
});
