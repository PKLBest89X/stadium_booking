...
import NonActiveBooking from "./NonActiveBooking";
...
const AllBookingUnPayment = React.memo(() => {
  ...
  const { userActiveData, userActiveSuccess, resultSearchAndSeletedDate } =
  ...
  useEffect(() => {
    const getReportBooking = async () => {
      ...
      if (userToken && userToken.token) {
        ...
        dispatch(onLoadUserActiveBooking("ຍັງບໍ່ເຕະ"));
      }
    };
    getReportBooking();
  }, [dispatch]);
  ...
  return (
    <ReportContainerLayout>
      ...
      <Box padding="1rem">
        ...
        {userActiveSuccess === false && <NonActiveBooking />}
      </Box>
    </ReportContainerLayout>
  );
});

export default AllBookingUnPayment;
