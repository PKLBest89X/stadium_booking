...
import { fetchAvailableCancel } from "../../../../middlewares/user/fetchBooking/fetchCancelBooking";
...
const CancelingBooking = React.memo(() => {
  ...
  useEffect(() => {
    ...
    if (userToken && userToken.token) {
      dispatch(fetchAvailableCancel(userToken.token));
    }
  }, [dispatch]);
  ...
  return (
    <ReportContainerLayout>
      ...
      <Box padding="1rem">
        ...
        {cancelSuccess === false && <NonDataCancelingBooking />}
      </Box>
    </ReportContainerLayout>
  );
});
