...
const TimesAndPrice = React.memo(({ times, filterData }) => {
  const classes = useStyles();
  const { stadiumId, bookingId } = useParams();
  const dispatch = useDispatch();
  const { bookingDetailsData, timeAndPriceSelected } = useShallowEqualSelector(
  ...
  useEffect(() => {
    dispatch(fetchGetBookingDetailsUnCheckout(stadiumId));
  }, [dispatch, stadiumId]);

  const stableSort = (array, bookingUncheckOut, comparator) => {
    let filterBooking = [];
    filterBooking = array.filter(
      (items1) =>
        !bookingUncheckOut.some((items2) => items1.std_id === items2.std_id)
    );
    const stabilizedThis = filterBooking.map((el, index) => [el, index]);
