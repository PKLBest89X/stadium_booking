...
const BookingTable = React.memo(({ bookingDetails }) => {
  ...
  const { bookingDetailsSelected } = useShallowEqualSelector(
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      dispatch(onSelectedAllBookingDetails(bookingDetails));
      return;
    }
    dispatch(onClearBookingDetails());
  };

  const handleClick = (event, name) => dispatch(onSelectedBookingDetails(name));
  ...
  const isSelected = (name) =>
    bookingDetailsSelected.findIndex(
